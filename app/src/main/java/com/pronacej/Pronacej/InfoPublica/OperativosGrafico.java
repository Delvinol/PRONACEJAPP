...
public class OperativosGrafico extends AppCompatActivity {
    ...
    private List<Map<String, Object>> operativosData;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.resultado_operativos);

        Button ButtonBack = findViewById(R.id.buttonBack);
        Button ButtonHome = findViewById(R.id.buttonHome);

        ButtonHome.setOnClickListener(v -> {
            Intent intentHome = new Intent(OperativosGrafico.this, CategoriaMenu.class);
    ...
    private void fetchOperativosData() {
        SeguridadService service = Apis.getSeguridadService();
        Call<List<Map<String, Object>>> call = service.getOperativos();

        call.enqueue(new Callback<List<Map<String, Object>>>() {
            @Override
            public void onResponse(Call<List<Map<String, Object>>> call, Response<List<Map<String, Object>>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    operativosData = response.body();
                    setupChart();
                } else {
                    Toast.makeText(OperativosGrafico.this, "Error al obtener datos", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(Call<List<Map<String, Object>>> call, Throwable t) {
                Toast.makeText(OperativosGrafico.this, "Error de conexión", Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void setupChart() {
        ...
        for (int i = 0; i < operativosData.size(); i++) {
            Map<String, Object> reyerta = operativosData.get(i);
        ...
        BarDataSet dataSet = new BarDataSet(entries, "Operativos Totales por cada centro CJDR");
        dataSet.setColor(ContextCompat.getColor(this, R.color.Pronacej6));
        ...
        xAxis.setValueFormatter(new ValueFormatter() {
            @Override
            public String getFormattedValue(float value) {
                if (value >= 0 && value < operativosData.size()) {
                    return (String) operativosData.get((int) value).get("centro_juvenil");
                }
                return "";
            }
        });
        ...
        xAxis.setLabelCount(operativosData.size());
